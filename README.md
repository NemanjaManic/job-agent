...
```
RemoteOK API → dedup (data/seen.jsonl) → Sloj 1: keyword filter → Sloj 2: LLM ocena → Telegram digest
```

1. **Dohvat** — `agent/sources/remoteok.py` povuče sve trenutne oglase sa RemoteOK API-ja.
...
- ⬜ Faza 2b — dodatni izvori (WeWorkRemotely, HelloWorld.rs, Infostud)
...
agent/
  ...
  llm.py                       Sloj 2 — ocena relevantnosti preko Gemini API-ja
  format.py                    format Telegram poruke
  telegram.py                  slanje na Telegram Bot API
  sources/remoteok.py          RemoteOK adapter
